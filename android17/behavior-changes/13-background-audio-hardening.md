...
| --- | --- | --- |
| Android 17 OS update only? | Conditional / Unknown | 公式文書は一部制限が all apps に適用されると述べるが、詳細と AOSP gate は未確認。 |
| targetSdkVersion 37 required? | Partially, but unverified | 原文は targetSdkVersion 37 以上では restrictions are more stringent と述べている。 |
| Additional runtime conditions? | Yes | background audio interaction、foreground service running、WIU capabilities、exact alarm permission、`USAGE_ALARM`。 |
