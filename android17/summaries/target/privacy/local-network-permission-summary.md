...
| --- | --- |
| Android 17 / targetSdkVersion 36 | 未確認。公式 docs 上は legacy app に temporary implicit grant があるが、AOSP gate 未確認。 |
...
| Android 17 | 36 | 未確認。legacy app は temporary implicit grant と説明されるが、AOSP gate 未確認。 |
...
- 検証対象の原文: Android 17 は `ACCESS_LOCAL_NETWORK` runtime permission を導入し、targetSdkVersion 37 以上のアプリは picker または runtime permission request により LAN communication を維持する必要がある。
...
- Gate conclusion: 未確認。公式文書は targetSdkVersion 37 以上と runtime / permission / picker 条件を示すが、AOSP gate evidence は未取得。
