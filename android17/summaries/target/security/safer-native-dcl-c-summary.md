...
- 検証対象の原文: targetSdkVersion 37 以上のアプリでは、Android 14 で DEX / JAR files に導入された Safer DCL protection が native libraries に拡張される。`System.load()` で読み込まれる native files は read-only 必須で、違反時は `UnsatisfiedLinkError`。
