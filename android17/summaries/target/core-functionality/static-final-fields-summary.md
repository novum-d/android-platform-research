# Static final fields are now unmodifiable - 1ページ要約（One Page Summary）
...
- OS アップデート / 全アプリ（OS update / all apps）: Unknown。公式文書は Android 17+ と targetSdkVersion 37+ の両方を条件としている。
- targetSdkVersion 37 以上: 公式文書上は該当。ただし AOSP gate 未確認。
- その他の必須条件（Other required conditions）: static final field を reflection または JNI で変更しようとする場合。
