# New lock-free implementation of MessageQueue - 1ページ要約（One Page Summary）
...
- OS アップデート / 全アプリ（OS update / all apps）: Unknown。公式文書は all apps ページではなく、apps targeting Android 17 or higher ページに掲載。
- targetSdkVersion 37 以上: 公式文書上は該当。ただし AOSP gate 未確認。
- その他の必須条件（Other required conditions）: `MessageQueue` private field / private method への reflection が互換性リスク条件。
