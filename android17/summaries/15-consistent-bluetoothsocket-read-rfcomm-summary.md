# Consistent BluetoothSocket read() behavior for RFCOMM - One Page Summary
...
- Other required conditions: RFCOMM-based `BluetoothSocket`、`InputStream.read()`、socket closed / connection dropped、read loop 実装。
...
| Android 17 / targetSdkVersion 37 + required conditions | `IOException` だけに依存する read loop は終了しない可能性があり、`-1` check が必要。 |
