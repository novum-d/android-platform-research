# SMS OTP protection - 1ページ要約（One Page Summary）
...
- Original statement: Android 17 から WebOTP format messages にも SMS OTP protection が適用され、intended recipient ではない app では受信後 3 時間まで access できない。遅延中は `SMS_RECEIVED_ACTION` broadcast が withheld され、SMS provider database queries が filtered される。
...
- Gate conclusion: Unknown。公式文書上は Android 17 all apps + WebOTP / SMS read permission / domain verification / 3 hour delay / exemption 条件。targetSdkVersion gate / compat framework evidence は未取得。
