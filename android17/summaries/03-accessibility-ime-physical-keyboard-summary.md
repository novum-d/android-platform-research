# Accessibility support of complex IME physical keyboard typing - 1ページ要約（One Page Summary）
...
- OS アップデート / 全アプリ（OS update / all apps）: Unknown。公式ページは targetSdkVersion 37+ 向けだが、API adoption 条件も含む。
- targetSdkVersion 37 以上: standard `TextView` の既定有効化は公式文書上 targetSdkVersion 37+。AOSP gate 未確認。
- その他の必須条件（Other required conditions）: CJKV IME composition、physical keyboard typing、custom `InputConnection`、`TYPE_VIEW_TEXT_CHANGED`、AccessibilityService の処理。
