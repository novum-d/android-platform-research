...
このセクションは、公式文書と AOSP evidence から導いた「実アプリで起こりうる影響シーン」を記録する。
特定サービスで実際に発生確認した事実ではない。

原則として 1〜3 個を記載する。
実サービス名を出す場合は、repository owner または service owner が確認済みのものだけにする。
それ以外は「メッセージングアプリの画像添付」「ログイン OTP」「Bluetooth プリンター連携」「連絡先 CRM 同期」のような一般化した service / feature pattern として書く。
...
- 根拠:
- Confidence（信頼度）:
- 注意: この例は一般化した影響シーンであり、特定アプリでの発生事実ではない。
