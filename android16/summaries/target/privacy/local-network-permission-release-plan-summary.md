...
- `OPT_IN_ONLY`はcurrent opt-in testingとfuture enforcement planのうち、現時点で実際に影響を発生させるopt-in条件を表す分類である。
...
- 対応する[主レポート](../../../behavior-changes/target/privacy/local-network-permission-release-plan.md)で official section、AOSP project / remote / checkout、解決済み commit、比較 command、dirty risk を再検証した。
