...
- targetSdkVersion 36 以上: Yes。`STPE_SKIP_MULTIPLE_MISSED_PERIODIC_TASKS` が Android 16 / API level 36 以上 target で default enabled。
- その他の必須条件（Other required conditions）: `scheduleAtFixedRate` を使い、process freeze / suspend 等で複数 period を missed してから復帰すること。
- Compat Change ID: `STPE_SKIP_MULTIPLE_MISSED_PERIODIC_TASKS` / 288912692
...
targetSdkVersion 36 以上では、`ScheduledThreadPoolExecutor#scheduleAtFixedRate` が missed period を複数回分まとめて即時 catch-up しない。
...
- 不要: WorkManager / JobScheduler / AlarmManager のみを使い、STPE fixed-rate catch-up に依存していない場合。
...
missed period の回数分だけ処理する必要がある場合は、executor の catch-up 回数に依存せず、最終処理時刻と現在時刻から必要な処理量を明示的に計算してください。
...
- AOSP files: `platform/libcore/ojluni/src/main/java/java/util/concurrent/ScheduledThreadPoolExecutor.java`、`platform/libcore/libcore.aconfig`、`platform/libcore/api/current.txt`、`frameworks-base/services/core/java/com/android/server/am/Freezer.java`
- AOSP source context: app API -> `scheduleAtFixedRate()` -> `ScheduledFutureTask#setNextRunTime()` -> `STPE_SKIP_MULTIPLE_MISSED_PERIODIC_TASKS` gate。
