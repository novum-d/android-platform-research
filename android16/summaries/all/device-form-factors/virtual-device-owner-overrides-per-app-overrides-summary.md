# Per-app overrides - 1ページ要約（One Page Summary）
...
- その他の必須条件: trusted / privileged virtual device owner が管理する selected virtual device / trusted virtual display で、activity size restrictions ignore が有効。
...
| --- | --- |
| Android 16 / targetSdkVersion 35 / local physical display | 本件の per-display projection override は原則影響なし。 |
| Android 16 / targetSdkVersion 36 / local physical display | targetSdkVersion 35 と同じ。 |
| Android 16 / targetSdkVersion 35 / projected by virtual device owner | selected trusted virtual display で override 有効なら制限が無視され得る。 |
...
Android 16 では、virtual device owner が管理する select virtual devices 上で app settings を override できる。代表例は、external display projection 時に app の orientation、aspect ratio、resizability restrictions を無視すること。

これは targetSdkVersion 36 化ではなく、Android 16 OS 上で trusted / privileged virtual device owner projection 経路に乗る場合の条件付き影響である。
...
| Android 16 / local display | 35 | 本件の per-display projection override は原則なし。 |
| Android 16 / local display | 36 | targetSdkVersion 35 と同じ。 |
| Android 16 / virtual device owner projection | 35 | selected trusted virtual display で override 有効なら制限が無視され得る。 |
...
| projection target | PC、VR、car infotainment、Chromebook、large display。 |
...
この変更は、Android 16 へ OS アップデートしたすべての通常起動で画面制限が無視されるという意味ではありません。影響が出るのは、trusted / privileged virtual device owner が app を virtual device 上で実行し、remote / external display に投影する場合です。

その投影環境では、virtual device owner が select virtual devices 上で app settings を override できるため、orientation、aspect ratio、resizability restrictions が無視される可能性があります。targetSdkVersion 36 へ上げたこと自体が直接条件ではありません。

## 根拠（Evidence）

- Official documentation: https://developer.android.com/about/versions/16/behavior-changes-all#per-app_overrides
- Parent documentation: https://developer.android.com/about/versions/16/behavior-changes-all#virtual-device-owner-overrides
...
- Diff interpretation:
  - 関連 API / flag は Android 15 tag にも存在するため、完全新規 API とは扱わない。
  - Android 16 公式 all apps behavior change として、projection 環境の app setting override impact を説明する。
- Gate conclusion:
  - Android 16、trusted / privileged virtual device owner、selected trusted virtual display、override enabled が主 gate。targetSdkVersion 36 は主 gate ではない。
...
- 対応する[主レポート](../../../behavior-changes/all/device-form-factors/virtual-device-owner-overrides-per-app-overrides.md)で official section、AOSP project / remote / checkout、解決済み commit、比較 command、dirty risk を再検証した。
