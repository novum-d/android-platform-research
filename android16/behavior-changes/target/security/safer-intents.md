...
Confidence note: 公式文書は Android 16 / targetSdkVersion 36 以上向けの behavior change として説明している。一方、AOSP の `android:intentMatchingFlags` enforcement path では明示的な targetSdkVersion 36 gate を確認できなかった。実装上の直接条件は Android 16 の feature flag、manifest opt-in、cross-app intent resolution であるため、confidence は Medium とする。
