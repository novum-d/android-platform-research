...
- appが固定方向、aspect ratio、サイズ変更可否の制約に依存している場合にUI impactが出やすい。
...
影響を受けやすいのは、portrait-only、固定方向、`resizeableActivity=false`、min/max aspect ratioなどに依存するアプリである。対応としては、制限指定を前提にせず、large screen / external display / arbitrary window sizeに耐えるadaptive layoutとQAが必要になる。
...
- `VirtualDisplayConfig.Builder#setIgnoreActivitySizeRestrictions(boolean)`は、virtual displayがappの固定方向、aspect ratio、サイズ変更可否の制約を無視するかを指定できる。
...
- AppCompat policyはそのdisplay flagを見て、eligible virtual display上のactivityからの画面の向きの要求を`SCREEN_ORIENTATION_USER`相当に扱い、aspect ratio / fullscreen override条件にも含める。
...
| --- | --- | --- | --- |
| `VirtualDisplayConfig#isIgnoreActivitySizeRestrictions()` / Builder method | `android-15.0.0_r36`にもflagged SystemApiとして存在 | `android-16.0.0_r4`でも固定方向 / aspect ratio / サイズ変更可否の制約を無視することを表すSystemApi | Per-app overridesの入口。公式文書の"ignore orientation, aspect ratio, and resizability restrictions"と一致する。 |
...
| `AppCompatOrientationPolicy#overrideOrientationIfNeeded` | eligible displayで画面の向きの要求を`SCREEN_ORIENTATION_USER`に変換 | Android 16 targetでも同経路 | screenOrientationがvirtual display owner overrideで無視される根拠。 |
...
`VirtualDisplayConfig#isIgnoreActivitySizeRestrictions()`は、virtual displayがappの固定方向、aspect ratio、サイズ変更可否の制約を無視するかを返す。Builder methodは`@SystemApi` / `@FlaggedApi`で、trueにするには`DisplayManager#VIRTUAL_DISPLAY_FLAG_TRUSTED`が必要であり、そうでなければpropertyは無視される。
...
`DisplayWindowSettings`は`mIgnoreActivitySizeRestrictions`をdisplay override settingsに保存し、`DisplayContent`は作成時にこの値を読み込む。`DisplayContent#isDisplayIgnoreActivitySizeRestrictions()`は固定方向、aspect ratio、サイズ変更可否の制約を無視するdisplayかを返す。
...
`AppCompatOrientationPolicy#overrideOrientationIfNeeded(...)`は、eligible virtual displayでactivityからの画面の向きの要求を無視し`SCREEN_ORIENTATION_USER`を返す。`AppCompatAspectRatioOverrides#hasFullscreenOverride()`はdisplay ignore conditionをfullscreen override条件に含める。`AppCompatUtils#isDisplayIgnoreActivitySizeRestrictions(...)`はaconfig flagとdisplay stateをgateにする。
...
- `OVERRIDE_ANY_ORIENTATION_TO_USER` / 310816437: disabled for all apps。package overrideによりactivityが要求した画面の向きを`SCREEN_ORIENTATION_USER`にできる。
...
- AOSPでは`VirtualDisplayConfig.Builder#setIgnoreActivitySizeRestrictions(boolean)`が固定方向、aspect ratio、サイズ変更可否の制約を無視することを表す。
...
- Android 16 production / OEM / companion app streaming implementationでこのoverrideが有効になると、従来はletterbox / 固定方向 / 固定aspect ratioで守られていたphone UIが、external display上で広いboundsに展開される可能性がある。
...
| シナリオ（Scenario） | 期待挙動（Expected behavior） |
| --- | --- |
| Android 16 / orientation restriction ignored | activityが要求した画面の向きはeligible virtual displayで`SCREEN_ORIENTATION_USER`相当に扱われ得る。 |
...
その投影環境では、virtual device ownerがselect virtual devices上でapp settingsをoverrideでき、画面の向き、aspect ratio、サイズ変更可否の制約が無視される可能性があります。したがって、portrait-only、固定方向、`resizeableActivity=false`、min/max aspect ratioを前提にしたUIは、large screen / landscape / desktop-class boundsで崩れる可能性があります。
